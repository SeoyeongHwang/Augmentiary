...
discover_template_v2 = PromptTemplate(
    ...
    template=(
        ...
        - {highlight}을 강조하되, 너무 거창한 해석은 지양
...
augment_template_v2 = PromptTemplate(
    ...
    template=(
        ...
        2. {life_orientation} 관점에서 의미/관점/시각을 확장할 때:
        - 원본 일기의 사실(사건, 행동, 감정)을 유지하세요.
        - 제시된 발췌문의 위치에 새로운 통찰을 덧붙이되, 관점 설명은 그대로 복사하지 말고 영감으로만 삼으십시오.
        - {highlight}에 집중하되, 원본 글의 내러티브, 감정적 연속성을 고려하여 너무 거창하게 쓰지 않도록 합니다.
        - 일상적이고 쉬운 표현을 쓰고 문장길이는 짧게 쓰세요. 스스로에게 제안하거나 질문하는 어조를 사용하세요.
